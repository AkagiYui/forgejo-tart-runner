...
| 数 | 含义 | 怎么看 |
|---|---|---|
| **Disk（容量）** | guest 磁盘总大小 = 上限，**含空闲空间** | `tart list` 的 Disk 列（base 默认 50） |
| **Size（已用内容）** | 镜像里已写入多少 ≈ guest 已有文件 | `tart list` 的 Size 列（或 `--format json`） |
| **guest 剩余可用** | 一个 job 实际还能写多少 | `tart exec <vm> df -h /` 的 Avail；约等于 `Disk − Size` |
| **宿主物理实占** | 真正吃掉的宿主空间 | 宿主 `df`（克隆是 CoW，几乎不额外占）；**别用 `du`**——它把 CoW 共享块重复计，虚高 |

> 实测 `tahoe-base`：Disk=50 / Size=34 → guest 里 `df` 只剩 **~15 GB 可用**，一个 job 余量
> 其实不多。所以「镜像 70 GB」要看是哪个数：是 **Disk** 就是盘的总容量（含空闲）；是下载大小 /
> **Size** / `du` 就是**已有文件**，剩余 = `Disk − Size`。不够就 `tart set <vm> --disk-size <更大>`
> 或换余量更大的镜像。
